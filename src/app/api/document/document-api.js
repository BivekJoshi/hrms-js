...
export const getDocumentById = async (id) => {
  const data = await axiosInstance.get(`/document/file-details-by-employee/${id}`);
  return data;
};

  /*________________________GET-BY-FILE-ID_____________________________________*/
  export const getDocumentByFileId = async (id) => {
    const data = await axiosInstance.get(`/document/file-details-by-file-id/${id}`);
    return data;
  };
...
export const getDocumentByDocumentType = async (id, documentType) => {
  if(id && documentType){
    const data = await axiosInstance.get(`/document/file-details-employee-and-document-type/${id}?documentType=${documentType}`);
    return data;
  }
