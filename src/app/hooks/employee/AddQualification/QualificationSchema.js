  ...
  education: Yup.array()
    .of(
      Yup.object().shape({
        ...
        passedYear: Yup.string().matches(/^[0-9]+$/, 'Enter valid passed year').required('Passed year is required'),
        grade: Yup.string().matches(/^[0-9]+$/, 'Enter valid grade or percentage').required('Grade is required'),
      })
    )
